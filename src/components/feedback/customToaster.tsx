...
import type { ToastData, ToastPosition } from "@/types/toast";
...
export default function CustomToaster() {
  ...
  useEffect(() => {
    const unsubscribe = toastSubscribe((newToast: ToastData) => {
  ...
  const groupedToasts = toasts.reduce<Record<string, ToastData[]>>((acc, t) => {
  ...
  const positionClass = (pos: ToastPosition): string => {
  ...
  const getIcon = (t: ToastData): React.ReactNode => {
  ...
  const getBgColor = (t: ToastData): string => {
    switch (t.type) {
      case "success":
  ...
  return (
    <>
      {Object.entries(groupedToasts).map(([position, items]) => (
        <div
          key={position}
          className={positionClass(position as ToastPosition)}
        >
          <AnimatePresence>
            {items.map((t) => (
              <motion.div
